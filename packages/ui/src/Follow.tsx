...
/** Follow toggle: a star that fills when pressed. Storage is the host's business. */
export function FollowButton({ pressed, onClick, labelOn = "Following", labelOff = "Follow", compact = false }: { pressed: boolean; onClick: () => void; labelOn?: string; labelOff?: string; compact?: boolean }) {
  return (
    <button
      ...
      onClick={onClick}
      className={`inline-flex items-center gap-1 text-sm leading-none ${pressed ? "text-celeste-deep" : "text-muted hover:text-ink"}`}
    >
