 ...
 */
export function SquadSheet({ columns, caption, LinkComponent = DefaultLink }: { columns: SheetColumn[]; caption?: ReactNode; LinkComponent?: LinkLike }) {
  let i = 0;
  return (
    <figure className="m-0 border-2 border-ink bg-surface">
      <div className="grid grid-cols-2 gap-x-6 gap-y-4 p-4 sm:grid-cols-4 sm:gap-x-8">
        {columns.map((col) => (
          <div key={col.title}>
            ...
            <ol className="text-sm">
              {Array.from({ length: col.slots }, (_, k) => {
                ...
                return (
                  <li key={n ? n.key : `empty-${k}`} className="sheet-row grid grid-cols-[1.25rem_1fr_auto] items-start gap-x-1 border-b border-rule py-1" style={{ animationDelay: delay }}>
                    <span className="num pt-0.5 font-mono text-[11px] text-muted">{n?.rank ?? ""}</span>
                    {n ? (
                      <span className="condensed min-w-0 leading-tight">
                        {n.href ? (
                        ...
                        {n.club && <span className="block truncate text-[11px] text-muted">{n.club}</span>}
                      </span>
                    ) : (
