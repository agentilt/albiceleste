...
export function PanelRows({ rows }: { rows: { key: string; left: ReactNode; right?: ReactNode }[] }) {
  return (
    <ol className="grid min-w-0 flex-1 auto-rows-fr" style={{ gridTemplateColumns: "minmax(0, 1fr)" }}>
      {rows.map((r) => (
        <li key={r.key} className="flex min-h-9 min-w-0 items-center gap-4 border-b border-rule text-sm last:border-b-0">
