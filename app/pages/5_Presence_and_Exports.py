...
st.subheader("First move abroad from an Argentine club")
st.caption("Transfermarkt CC0 snapshot to June 2026, every Argentine in the snapshot, not only the players tracked above.")
yr_from = st.slider("From year", 2000, 2026, 2016)
dest = q(
    "select to_country, sum(players_exported) as players_exported from marts.argentine_export_summary where transfer_year >= ? group by 1 order by 2 desc limit 12",
    (yr_from,),
)
c1, c2 = st.columns(2)
with c1:
...
with c2:
    per_year = q(
        "select make_date(transfer_year, 1, 1) as year, sum(players_exported) as players_exported, round(avg(avg_age_at_export), 1) as avg_age from marts.argentine_export_summary where transfer_year >= ? group by 1 order by 1",
        (yr_from,),
    )
...
st.dataframe(
    ...
    hide_index=True,
    column_config={
        "exported_last_10y": st.column_config.NumberColumn("exported, last 10 years"),
        "avg_age_at_export": st.column_config.NumberColumn("avg age at export", format="%.1f"),
        "total_fees_eur": st.column_config.NumberColumn("total fees €", format="%,.0f"),
    },
)
