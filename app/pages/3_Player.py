...
players = q(
    ...
    from marts.dim_player where in_tracked_squad or eligibility_status = 'eligible' and identity_source = 'wikidata' and has_arg_senior_cap
...
key = st.selectbox("Player", keys, index=idx, format_func=lambda k: labels[k])
st.query_params["player_key"] = key

d = q("select * from marts.dim_player where player_key = %s", (key,))
if d.empty:
    st.stop()
p = d.iloc[0]

c1, c2, c3, c4, c5, c6 = st.columns(6)
c1.metric("Age", fmt_int(p["age"]))
c2.metric("Position", p["primary_position"] or "–")
c3.metric("Club", p["current_team_name"] or "–")
c4.metric("Competition", p["current_competition"] or "–")
c5.metric("Eligibility", p["eligibility_status"])
c6.metric("Market value", f"€{p['market_value_eur']/1e6:.0f}m" if pd.notna(p["market_value_eur"]) else "–")
flags = []
if p["has_arg_senior_cap"]:
    flags.append("senior international")
...
with tab1:
    season = q(
        ...
        "from marts.player_season_stats where player_key = %s order by season_year desc, minutes desc",
        (key,),
    )
    ...
    matches = q(
        ...
        where f.player_key = %s order by f.match_date desc limit 40
        """,
        (key,),
    )
...
with tab2:
    form = q(
        ...
        "from marts.player_recent_form where player_key = %s order by window_days",
        (key,),
    )
    st.subheader("Rolling windows vs the preceding window")
    st.dataframe(form, width="stretch", hide_index=True)
    ev = q("select event_date, severity, event_type, headline from marts.player_events where player_key = %s order by event_date desc, severity desc", (key,))
    st.subheader("Events")
    st.dataframe(ev, width="stretch", hide_index=True)

with tab3:
    hist = q(
        "select season, competition_name, club_name, appearances, minutes, goals, assists from marts.player_season_history where player_key = %s order by season desc, minutes desc",
        (key,),
    )
    ...
    moves = q(
        ...
        "from marts.player_career_history where player_key = %s order by transfer_date",
        (key,),
    )
    st.subheader("Transfers")
    st.dataframe(moves, width="stretch", hide_index=True)
    mv = q("select valuation_date, market_value_eur from marts.player_market_values where player_key = %s order by 1", (key,))
    if not mv.empty:
        mv["market_value_m"] = mv["market_value_eur"] / 1e6
        line(mv, "valuation_date", "market_value_m", "Market value (€m)", "€m", fmt=".0f")

with tab4:
    ids = q("select source, source_id from marts.player_source_ids where player_key = %s order by source", (key,))
    st.dataframe(ids, width="stretch", hide_index=True)
    st.caption(f"Identity source: {p['identity_source']} · eligibility basis: {p['eligibility_basis']} · squad snapshot: {p['squad_as_of']}")
